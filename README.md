...
> La commande de l’outil Openscap ci-dessous va lancer une analyse via ssh sur la machine cible « srv2 » en se basant sur les critères de sécurité ANSSI pour ensuite générer des fichiers de rapport en xml et html qui seront nommés avec le contenu de la variable $type c'est à dire « srv2-bp28m-before ».
...
> En exécutant la commande suivant ci-dessous nous allons charger le fix ansible dans une variable nommée $result_id, pour cela nous avons besoin de l’id de la règle qui est stocké dans la variable $rule1  :
